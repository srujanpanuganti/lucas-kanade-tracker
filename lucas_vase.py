...
def iterative(gray1,points,T,P,count):
    ...
    # while(norm >= 0.01):# and norm <= 0.00):
        ...
        # hessian =  hessian + np.dot(steepest_descent_reshaped.T,steepest_descent_reshaped)
        ...
        #     gamma = np.diag([1, 1,1, 1, -0.2, 0.2])
        # if count >10 and count <30:
        #     gamma = np.diag([1,1,1,1,-2.5,2.5])
        # else:
        #     gamma = np.diag([1,1,1,1,-1.5,1.1])
        #
...
img1 = cv2.imread('vase/0019.jpg')  ### image at time step 1
gray1 = cv2.cvtColor(img1,cv2.COLOR_BGR2GRAY)

# cropped = gray[72:148, 123:172]
points = [[123,72],[172,148]]
...
######## obtain the frames
filenames = [img for img in glob.glob("vase/*.jpg")]
filenames.sort()
